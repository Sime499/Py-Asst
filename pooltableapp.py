...
def display_tables():
    ...
    for table in tables:
        if table.occupied==False:
            status="Available"
            print(f"Table {table.number}: {status}  current_time--{table.current_time.strftime('%m/%d/%Y %H:%M%:%S')}")
...
while True:
    ...
    try:
        if choice== "1":
            ...
            for table in tables:
                print(f"Table No--{table.number}--{status}-- current_time--{current_time.strftime('%m/%d/%Y %H:%M%:%S')}\n")
